...
df = pd.read_csv('original_data_set/2022_Rental_data.csv', dtype={'月租金': int})
print('原始資料:',len(df.index))
df=df.loc[(df['縣市'] == 17) | (df['縣市'] == 11)]
print('只有雙北:',len(df.index))
df=df.loc[(df['房屋曾出租過'] == 2)]
print('只有雙北,且出租過:',len(df.index))
df=df.loc[(df['物件類型'] == 1)|(df['物件類型'] == 2)|(df['物件類型'] == 3)]
print('只有雙北,且出租過的獨立套房、套房、雅房、:',len(df.index))
#print(len(df.index))
